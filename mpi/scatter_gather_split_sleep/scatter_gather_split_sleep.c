...
int main(int argc,char *argv[]){
	...
	count=40; /* cada proceso recibe count elementos del root */

	myray=(int*)malloc(count/numnodes*sizeof(int));
	if(myid == root){ 
	...
	mpi_err=MPI_Scatter(send_ray,count/numnodes,MPI_INT,myray,count/numnodes,MPI_INT,root,MPI_COMM_WORLD);

	/* cada proceso realiza una suma local */
	total=0;
	for(i=0;i<count/numnodes;i++){
		total=total+myray[i];
		sleep(1);
	}
