...
ExtraeHome     = "/home/jgimenez/tools/extrae-3.6.1"
  ...
  if ((event == "call") or (event == "c_call") or (event == "return") or (event == "c_return")):
    ...
    if Extrae and (os.getpid() in Extrae) and function and Traced_Functions and function in Traced_Functions:   
      ...
      #  print ("-> called from "+caller_filename+" line "+str(caller_line_no))
       ...
       Extrae[os.getpid()].Extrae_eventandcounters( USRFUNC_EV, function_id )
...
def eventandcounters(type, value):
  if (Extrae):
    Extrae[os.getpid()].Extrae_eventandcounters(type, value)
...
def neventandcounters(array_of_types, array_of_values):
  if (Extrae):
    ...
    for i in range(0, nevents):
      types[i] = array_of_types[i]
      values[i] = array_of_values[i]
    Extrae[os.getpid()].Extrae_neventandcounters(nevents, ctypes.pointer(types), ctypes.pointer(values))
